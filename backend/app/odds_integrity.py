...
def check_and_fix_odds(db: Session):
    ...
    Uses query batching to prevent N+1 queries and a retry loop to handle deadlocks.
    """
    max_retries = 3
    initial_delay = 0.2
    delay = initial_delay
    ...
    for attempt in range(max_retries):
        try:
            ...
            if not active_matches:
                # Even if empty, commit/rollback to close the transaction block
                db.commit()
                return

            active_match_ids = [m.id for m in active_matches]
            
            # Batch query all odds for these matches in one go
            all_odds = db.query(Odd).filter(Odd.match_id.in_(active_match_ids)).all()
            ...
            for match in active_matches:
                match_odds = odds_by_match.get(match.id, [])
                if not match_odds:
                    continue
            ...
            # Successfully finished, break out of retry loop
            break

        except DBAPIError as e:
            # Check if underlying exception is a deadlock error (e.g. SQLState 40001, Error 1205)
            is_deadlock = False
            if e.orig and hasattr(e.orig, 'args'):
                for arg in e.orig.args:
                    if isinstance(arg, str) and ('40001' in arg or '1205' in arg or 'deadlock' in arg.lower()):
                        is_deadlock = True
                        break
                    elif isinstance(arg, int) and arg == 1205:
                        is_deadlock = True
                        break
            ...
            if is_deadlock and attempt < max_retries - 1:
                ...
                logger.warning(f"Deadlock detected in check_and_fix_odds. Retrying in {sleep_time:.2f} seconds (Attempt {attempt+1}/{max_retries})...")
