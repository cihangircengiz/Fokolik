...
def process_mackolik_matches(db: Session, days_forward: int = 3):
    ...
    for date_str in dates_to_fetch:
        ...
        for m_id, mdata in matches_dict.items():
            ...
            iddaa_codes.append(iddaa_code_str)
            fetched_matches_data.append((iddaa_code_str, mdata, competitions_dict))

    if not fetched_matches_data:
        return [], []

    # Batch query all existing matches in one go
    db_matches = db.query(Match).filter(Match.id.in_(iddaa_codes)).all()
    db_matches_map = {m.id: m for m in db_matches}

    updated_matches_data = []
    newly_finished_ids = []

    for iddaa_code_str, mdata, competitions_dict in fetched_matches_data:
        # Get league name
        ...
        # Retrieve match from the map
        db_match = db_matches_map.get(iddaa_code_str)
        ...
        if not db_match:
            ...
            db.add(db_match)
            db_matches_map[iddaa_code_str] = db_match
            # Ensure we have the match flushed to the database
            db.flush()
